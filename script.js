...
function attachEventListeners() {
    ...
    document.querySelectorAll('.task-card').forEach(card => {
        card.addEventListener('click', (e) => {
            e.preventDefault();
            selectTaskType(e.currentTarget);
        });
    });
    console.log('Task card listeners attached');
...
function selectTaskType(card) {
    ...
    console.log('Task type selected:', state.taskType);
}
...
function toggleFewShotSection() {
    ...
    if (fewshotCheck && fewshotSection) {
...
function resetForm() {
    ...
    const profRadio = document.querySelector('input[name="tone"][value="professional"]');
    if (profRadio) profRadio.checked = true;

    const preview = document.getElementById('promptPreview');
    if (preview) {
        preview.innerHTML = '<p class="preview-placeholder">Your generated prompt will appear here. Fill in the form and click "Generate Prompt" to start.</p>';
    }
...
function loadTemplates() {
    ...
    if (!templateGrid || typeof promptTemplates === 'undefined') {
    ...
    document.querySelectorAll('.template-item').forEach(item => {
        item.addEventListener('click', function() {
            ...
            loadTemplate(promptTemplates[index]);
        });
    });
